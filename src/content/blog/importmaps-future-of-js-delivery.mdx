...
In this post, I want to cover briefly what ES Modules are, what they can replace, as well as the historical reasons why we've used bundlers in the past, and why
...
Without getting too into the weeds on the nuances of HTTP, suffice it to say that in HTTP/1.x, the relationship between requests and the underlying TCP connection is 1:1. That is, only one resource may be loaded for each TCP connection.

The issue is that if you're using TLS (and I hope you are), the client needs to make at least two round trips before the request is made simply to establish the connection, before any bytes relevant to your application can be served.
...
Because we're doing a new TLS handshake for every request, as highlighted above, that's at least 2x more server roundtrips for every additional resource!
...
The above figure demonstrates JS activity by category for a 437kb bundle from [factor.fyi](https://factor.fyi). The lion's share of time, **382ms** is due to "scripting"
...
While using ES Modules to serve your JS isn't going to make parsing faster, it provides a built-in mechanism for code-splitting, helping reduce the
...
<h2 slot="header">
  ...
  <slot>
    ...
    Why not use `importmap-rails`? Well, it famously does not, <a href={"https://github.com/rails/importmap-rails/issues/124#issuecomment-1113969386"} target={"_blank"}>nor will not</a>, support Typescript. Luckily, it's not that hard to roll yourself using sprockets and esbuild.
  </slot>
</Aside>
...
One thing to note, however, is that all imports of local modules must be non-relative. The reason for this is that modules declared in the import map are all
declared as non-relative. Using relative paths in your source code would force you to add a distinct importmap entry for every possible relative path to a module.
Since the browser would treat all of these as different files from a caching perspective, a better approach is to just ensure that you're always using the same import for a module
that would appear in the import map.
...
// We need to tell typescript how to
// resolve this non-relative path
...
I like to keep my builld system really simple
...
1. Server load: you don't waste your own CPU cycles delivering someone else's code.
1. Latency: CDN edge nodes are typically going to be a lot closer to your users than your server is.
1. They're totally free to use

There are bunch of really great ESModule CDNs out there, but I use [esm.sh](https://esm.sh).
esm.sh has some really cool features, like the ability to alias dependencies to support things like <a href="https://esm.sh/#docs" target="_blank">preact/compat</a>, and
